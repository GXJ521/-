...
@property(nonatomic,copy)NSArray *resultIDArr;
...
-(instancetype)initPickviewWithArray:(NSArray *)array initPickId:(NSArray *)arrID isHaveNavControler:(BOOL)isHaveNavControler{
    self=[super init];
    if (self) {
        
        
        self.plistArray=array;
        self.resultIDArr =arrID;
        [self setArrayClass:array setArrID:arrID];
        [self setUpPickView];
        [self setFrameWith:isHaveNavControler];
    }
    return self;
...
-(void)setArrayClass:(NSArray *)array setArrID:(NSArray *)arrID{
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    datePicker.minimumDate = [formatter dateFromString:@"1930年01月01日"];
    datePicker.maximumDate = [formatter dateFromString:@"2100年12月31日"];
    if (_defaulDate) {
...
-(void)doneClick
{
    if (_pickerView) {
        ...
        _resultString=[NSString stringWithFormat:@"%@",[DateUtil stringFromDate:_datePicker.date]];
        
    }
    
    if ([self.delegate respondsToSelector:@selector(toobarDonBtnHaveClick:resultString:resultID:)]) {
        
        [self.delegate toobarDonBtnHaveClick:self resultString:_resultString resultID:_resultID];
    }
    [self removeFromSuperview];
}
/**
